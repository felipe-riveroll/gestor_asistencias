...
async function fetchAllBranchesData(startDate, endDate) {
    ...
    // ===========================================
    // --- CORRECCIÓN (Sintaxis) ---
    console.log(`Cargando datos de 31PTE de ${startDate} a ${endDate} desde la API...`);
...
function updateEmployeeSummaryTable() {
    ...
    if (employeeData.length === 0) {
        ...
        // --- CORRECCIÓN (Sintaxis) ---
        tbody.innerHTML = `<tr_><td colspan="${colspanValue}" style="text-align:center;">${message}</td></tr_>`;
        return;
    }
...
function updateAllCharts(data) { 
    ...
    // --- Gráfica 1: Eficiencia por Sucursal (Ahora solo mostrará 1 barra de 31pte) ---
...
async function exportToExcelWithStyles() {
    ...
    // --- CORRECCIÓN (Sintaxis) ---
    const filename = `Dashboard_Analisis_31pte_${startDate}_a_${endDate}.xlsx`; // Nombre de archivo actualizado
