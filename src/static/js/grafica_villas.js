...
async function fetchAllBranchesData(startDate, endDate) {
    ...
    // ===========================================
    console.log(`Cargando datos de VILLAS de ${startDate} a ${endDate} desde la API...`);
    const apiUrl = `/api/dashboard/villas/?startDate=${startDate}&endDate=${endDate}`;
...
function updateDashboard(data) {
    ...
    // updateSummaryTable(data); // No se usa en esta página
...
function showNoData() { 
    ...
    // Ajustado a 7 columnas
    document.getElementById('employeeKpiTableBody').innerHTML = `<tr><td colspan="7" style="text-align:center;">No hay datos.</td></tr>`; 
...
function initializeCharts() { 
    ...
    charts.topEfficiencyChart = new Chart('topEfficiencyChart', { 
        ...
        options: { 
            ...
            indexAxis: 'y', 
            plugins: { legend: { display: false } }, 
            scales: { x: { beginAtZero: true } } // Quitamos el max: 100
        } 
    }); 
...
function updateAllCharts(data) { 
    ...
    // --- Gráfica 1: Eficiencia por Sucursal (Ahora solo mostrará 1 barra de Villas) ---
    ...
    // CORRECCIÓN: Esta gráfica debe basarse en los empleados, no en las sucursales
    ...
    // --- Gráfica 5: Comparativa (No se usa, pero la dejamos) ---
    ...
    charts.branchComparisonChart.data = { 
        ...
        datasets: branches.slice(0, 4).map((b, i) => { 
            return { 
                ...
                backgroundColor: 'rgba(133, 30, 35, 0.5)', // Color de Villas
...
async function exportToExcelWithStyles() {
    ...
    const filename = `Dashboard_Analisis_Villas_${startDate}_a_${endDate}.xlsx`; // Nombre de archivo actualizado
